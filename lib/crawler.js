...
var events = require("events"),
    debug = require("debug")("crawl:lib:crawler"),
...
crawler.crawl = function(url, options, callback) {
  ...
  // do a web crawl of url if it's an http protocol
    ...
    var port = urlParts.port ? urlParts.port : 80,
        siteCrawler = new Crawler(url);

    // configure crawler
    siteCrawler.interval = options.interval || 10;
    siteCrawler.maxConcurrency = options.maxConcurrency ||  10;
    siteCrawler.scanSubdomains = options.scanSubdomains || true;
    siteCrawler.respectRobotsTxt = options.respectRobotsTxt == undefined ? true : options.respectRobotsTxt;
    siteCrawler.downloadUnsupported = options.downloadUnsupported || false;
    if (options.maxDepth) {
      siteCrawler.maxDepth = options.maxDepth;
    }
    if (options.discoverRegex) {
      siteCrawler.discoverRegex = options.discoverRegex;
    }
    ...
    siteCrawler.on("discoverycomplete", function(queueItem, resources) {
      ...
      debug("discoverycomplete", parsedUrl);
